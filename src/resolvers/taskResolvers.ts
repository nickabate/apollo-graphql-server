import { readTasksFromFile, writeTasksToFile, Task } from "../utils/utils.js";
import { v4 } from "uuid";
...
export const resolvers = {
  ...
  Mutation: {
    ...
    createTask: (_: any, { text }: { text: string }) => {
      const tasks = readTasksFromFile();
    ...
    markTaskAsCompleted: (_: any, { id }: { id: string }) => {
      const tasks = readTasksFromFile();
      const task = tasks.find((task) => task.id === id);
      if (!task) {
        throw new Error("Task not found");
